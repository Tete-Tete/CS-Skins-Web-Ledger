...
exchange_rate = st.sidebar.number_input("当前汇率（¥ / $）", value=7.17, step=0.0001, format="%.5f")

st.sidebar.markdown("#### 💳 CSFloat 信用卡充值计算器")
card_amount = st.sidebar.number_input("输入充值金额（$）", value=0.0, step=0.01)

if card_amount > 0:
    card_fee = card_amount * 0.028 + 0.30
    total_payment = card_amount + card_fee
    st.sidebar.info(f"手续费：${card_fee:.2f}")
    st.sidebar.success(f"需支付总额：${total_payment:.2f}")

    # 💰 使用输入的汇率进行人民币换算
    total_rmb = total_payment * exchange_rate
    st.sidebar.warning(f"≈ ￥{total_rmb:.2f}")
...
if not filtered_data.empty:
    for idx, row in filtered_data.iterrows():
        ...
        if pd.isna(row['实际到手价格']) or row['实际到手价格'] == 0:
            color = "#ffcc00"
            status = "【未售出】"
        elif row['毛利'] > 0:
            color = "#ff4d4d"
        ...
        buy_date_str = pd.to_datetime(row["购入时间"]).strftime("%Y-%m-%d") if pd.notna(row["购入时间"]) else "—"
        profit_str = f"￥{row['毛利']:.2f}" if pd.notna(row['毛利']) else "—"
        font_family = "Segoe UI, Microsoft YaHei, sans-serif"

        with col1:
            st.markdown(
                f"<div style='color:{color}; font-family:{font_family}; font-size:16px; font-weight:500;'>"
                f"【{idx}】{status} {row['购买物品']} | 平台: {row['购买平台']} | "
                f"磨损: {row['磨损']:.3f}（{row['磨损等级']}） | 购入: ￥{row['购入价格']:.2f} | "
                f"购入时间: {buy_date_str} | 毛利: {profit_str}"
                f"</div>",
                unsafe_allow_html=True
            )
